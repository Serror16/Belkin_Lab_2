...
class System_Shell:
    ...
    def __init__(self):
        ...
        настраивает логирование, создает корзину и загружает историю."""
        ...
        try:
            ...
            if not flag_l:
                ...
                for elem in elems:
                    ...
                    print(
                        f"{elem} \t{stat.st_size}\t{datetime.fromtimestamp(stat.st_mtime)}\t{oct(stat.st_mode)[-3:]}"
                    )
        ...
        try:
            ...
            if os.path.dirname(rel_path) and os.path.dirname(rel_path) != ".":
                ...
                if not os.path.exists(trash_subdir):
                    os.makedirs(trash_subdir, exist_ok=True)  # Добавили exist_ok=True
    ...
    def run(self):
        ...
        while True:
            try:
                command = (
                    input(f"{Colors.BRIGHT_GREEN}{self.current_dir}{Colors.RESET} $ ")
                    .strip()
                    .split()
                )
                if not command:
